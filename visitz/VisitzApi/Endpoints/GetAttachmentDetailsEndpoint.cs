...
internal class GetAttachmentDetailsEndpoint(string baseUrl, ApiRecordType type, string rowId, DateTimeOffset? after = null)
    : VisitzBaseEndpoint<IEnumerable<AttachmentDetailJson>>(baseUrl, Vpi.V2, MakePath(type, rowId))
{
    ...
    public override IEnumerable<AttachmentDetailJson> HandleResponse(HttpResponseMessage response, string responseContent)
    {
        ...
        return JsonSerializer.Deserialize<IEnumerable<AttachmentDetailJson>>(items, PayloadOptions.SiebelGet);
    }
}
